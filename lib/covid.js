...
module.exports = async () => {
    ...
    table.push(
      [`${colors.rainbow('Total Countries')}: ${chalk.bold.inverse(re.byCity.length)}`, `${chalk.bold.inverse(re.confirmedTotal)}`, `${chalk.bold.inverse(re.deathTotal)}`, `${chalk.bold.inverse(re.recoveredTotal)}`]
    )
    re.byCity.forEach(c => {
    table.push(
      [`${c.attributes.Country_Region}`, `${c.attributes.Confirmed}`, `${c.attributes.Deaths || 0}`, `${c.attributes.Recovered || 0}`]
    );})
    console.log(`
